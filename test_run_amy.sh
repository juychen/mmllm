...
python run_sample_size_experiments.py \
  ...
  --m5c-bedgraph /data2st1/junyi/output/llm0401/processed_meth/MW_${region}.CG.m.bedGraph.gz \
  --hm5c-bedgraph /data2st1/junyi/output/llm0401/processed_meth/MW_${region}.CG.h.bedGraph.gz \
  --atac-bw /data2st2/junyi/output/atac1112/tobiasbam/BULK/corrected/${region}_MW_track.bw \
  --scheduler cosine \
  --num-epochs 100 \
  --batch-size 64 \
...
python run_atac_query_sequence_context_experiments.py \
  --sample-sizes 2000 20000 100000 \
  --scheduler cosine \
  --hm5c-bedgraph /data2st1/junyi/output/llm0401/processed_meth/MW_${region}.CG.m.bedGraph.gz \
  --atac-bw /data2st2/junyi/output/atac1112/tobiasbam/BULK/corrected/${region}_MW_track.bw \
  --num-epochs 100 \
  --batch-size 64 \
  --scheduler-patience 5 \
